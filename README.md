...
- **汉堡菜单**：大小滑块、音效套装（小黄鸭 / 叮叮咚咚 / 自定义组合）、音量、气泡开关、每轮消耗开关与自动关闭秒数、任务结束音、数据源切换、周额度与 5h 预警线（百分比口径）、边缘吸附开关、隐藏菜单按钮
- **右键唤出菜单**：菜单按钮可隐藏，隐藏后右键兔娘唤出
- **点按推进泡泡队列**：连点兔娘/泡泡按「状态 → 随机台词×3 → 收起」逐段推进
- **额度预警**：周剩余低于预警线、或 5h 窗口已用达预警线（官方百分比口径）时，兔娘主动冒泡着急
- **任务结束音**：一轮对话完成时播放提示音（经验球 / 音效 A，可开关）
- **用量记录窗口**：今日合计、近 7 天逐日、全时段模型占比、逐日明细，支持日期/模型名搜索
- **资源库**：角色图库（多图添加/启用/删除）、音效片段库（导入/试听/自由组合按压松手音效）、泡泡图库（内置 petpet/money1，可上传 gif）
